        ...
        <div style={{ maxWidth: 1180, margin: "0 auto" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: "1.5rem" }}>
            {articles.map((article) => (
              <article key={article.slug} style={{ background: "#fff", border: "1px solid #e1e7ee", borderRadius: 8, padding: "1.5rem", display: "flex", flexDirection: "column" }}>
                <p style={{ fontSize: "0.75rem", color: "#2968A0", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: "0.6rem" }}>{formatDate(article.datePublished)} · Boulder BioLabs</p>
                ...
                <Link href={`/resources/${article.slug}`} style={{ color: "#2968A0", fontWeight: 600, textDecoration: "none", fontSize: "0.9rem", marginTop: "auto" }}>Read article →</Link>
              </article>
            ))}
          </div>
          ...
          <div style={{ marginTop: "3.5rem" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: "1.2rem" }}>
              {pdfs.map((p) => (
                <article key={p.slug} style={{ background: "#fff", border: "1px solid #e1e7ee", borderLeft: "4px solid #2968A0", borderRadius: 8, padding: "1.4rem 1.4rem 1.2rem", display: "flex", flexDirection: "column" }}>
                  <p style={{ fontSize: "0.7rem", color: "#2968A0", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: "0.4rem" }}>Free PDF &middot; {p.pages} pages</p>
                  ...
                  <Link href={`/resources/${p.slug}`} style={{ color: "#2968A0", fontWeight: 600, textDecoration: "none", fontSize: "0.88rem", marginTop: "auto" }}>Get the PDF &rarr;</Link>
                </article>
              ))}
            </div>
          </div>
          ...
          <div style={{ background: "#F2F4F7", borderRadius: 8, padding: "2rem", marginTop: "3rem", textAlign: "center" }}>
            ...
            <Link href="/contact" style={{ display: "inline-block", background: "#2968A0", color: "#fff", padding: "0.8rem 1.6rem", borderRadius: 4, textDecoration: "none", fontWeight: 600 }}>Request a Quote</Link>
          </div>
        </div>
      </section>
    </>
