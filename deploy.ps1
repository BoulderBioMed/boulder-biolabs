# Boulder BioLabs - stage, commit, push, and trigger Vercel deploy in one shot.
#
# Usage:
#   powershell -ExecutionPolicy Bypass -File deploy.ps1 -Message "commit msg"
#   powershell -ExecutionPolicy Bypass -File deploy.ps1 -Message "commit msg" -SkipCommit
#
# Why this exists: BBL Vercel project's GitHub link is broken (Vercel user
# jimkasic-1023 OAuth'd to jim0425; repos owned by BoulderBioMed which
# jim0425 cannot see). Until that's fixed, push events from GitHub don't
# reach Vercel. This script pushes and then explicitly POSTs to the Vercel
# deployments API to start a production build from the new HEAD.
...
$projectId   = 'prj_yzqCDHXRDIIDbY0D1CVimA7d64Fq'
$teamId      = 'team_cJTblZcxdRH2BIIzWg5IHhu1'
$projectName = 'boulder-biolabs'
...
function Invoke-Git {
  ...
  try {
    # Quote each arg containing whitespace so Start-Process passes them as one arg.
...
# Clear stale Windows-side git-maintenance locks.
...
if (-not $SkipCommit) {
  ...
  # Drop whitespace-only churn on these files (CRLF/LF noise from Linux mount).
